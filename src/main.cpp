...
int Audio;
int Button = 0;
int Buttonflag = 0, Chooseflag = 0, Modflag = 0;
int Mod = 0;
float intonation = 0;
int string;
float freq = 0;
...
/*-------------------------------------------------------------------------------------------------------------------------------------------*/
bool InRange(float frequency, int low_limit, int high_limit){
...
void MusicalAlphabet(){
  ...
  tft.setCursor(70, 53);
  /*第六弦*/
  if(InRange(freq, 65.0, A2)){
    string = 6;
    if(InRange(freq, 77.8, F2)){
      intonation = map(freq, 77.8, F2, 0, 400); //E2
      tft.print("E ");
    }
    else if(InRange(freq, E2, F2U)){
      intonation = map(freq, E2, F2U, 0, 400); //F2
      tft.print("F ");
    }
    else if(InRange(freq, F2, G2)){
      intonation = map(freq, F2, G2, 0, 400); //F2U
      tft.print("F#");
    }
    else if(InRange(freq, F2U, G2U)){
      intonation = map(freq, F2U, G2U, 0, 400); //G2
      tft.print("G ");
    }
    else if(InRange(freq, G2, A2)){
      intonation = map(freq, G2, A2, 0, 400); //G2U
      tft.print("G#");
    }
  }
  /*第五弦*/
    ...
    if(InRange(freq, G2U, A2U)){
      intonation = map(freq, G2U, A2U, 0, 400); //A2
      tft.print("A ");
    }
    else if(InRange(freq, A2, B2)){
      intonation = map(freq, A2, B2, 0, 400); //A2U
      tft.print("A#");
    }
    else if(InRange(freq, A2U, C3)){
      intonation = map(freq, A2U, C3, 0, 400); //B2
      tft.print("B ");
    }
    else if(InRange(freq, B2, C3U)){
      intonation = map(freq, B2, C3U, 0, 400); //C3
      tft.print("C ");
    }
    else if(InRange(freq, C3, D3)){
      intonation = map(freq, C3, D3, 0, 400); //C3U
      tft.print("C#");
    }
  }
  /*第四弦*/
    ...
    if(InRange(freq, C3U, D3U)){
      intonation = map(freq, C3U, D3U, 0, 400); //D3
      tft.print("D ");
    }
    else if(InRange(freq, D3, E3)){
      intonation = map(freq, D3, E3, 0, 400); //D3U
      tft.print("D#");
    }
    else if(InRange(freq, D3U, F3)){
      intonation = map(freq, D3U, F3, 0, 400); //E3
      tft.print("E ");
    }
    else if(InRange(freq, E3, F3U)){
      intonation = map(freq, E3, F3U, 0, 400); //F3
      tft.print("F ");
    }
    else if(InRange(freq, F3, G3)){
      intonation = map(freq, F3, G3, 0, 400); //F3U
      tft.print("F#");
    }
  }
  /*第三弦*/
    ...
    if(InRange(freq, F3U, G3U)){
      intonation = map(freq, F3U, G3U, 0, 400); //G3
      tft.print("G ");
    }
    else if(InRange(freq, G3, A3)){
      intonation = map(freq, G3, A3, 0, 400); //G3U
      tft.print("G#");
    }
    else if(InRange(freq, G3U, A3U)){
      intonation = map(freq, G3U, A3U, 0, 400); //A3
      tft.print("A ");
    }
    else if(InRange(freq, A3, B3)){
      intonation = map(freq, A3, B3, 0, 400); //A3U
      tft.print("A#");
    }
  }
  /*第二弦*/
    ...
    if(InRange(freq, A3U, C4)){
      intonation = map(freq, A3U, C4, 0, 400); //B3
      tft.print("B ");
    }
    else if(InRange(freq, B3, C4U)){
      intonation = map(freq, B3, C4U, 0, 400); //C4
      tft.print("C ");
    }
    else if(InRange(freq, C4, D4)){
      intonation = map(freq, C4, D4, 0, 400); //C4U
      tft.print("C#");
    }
    else if(InRange(freq, C4U, D4U)){
      intonation = map(freq, C4U, D4U, 0, 400); //D4
      tft.print("D ");
    }
    else if(InRange(freq, D4, E4)){
      intonation = map(freq, D4, E4, 0, 400); //D4U
      tft.print("D#");
    }
  }
  /*第一弦*/
    ...
    if(InRange(freq, D4U, F4)){
      intonation = map(freq, D4U, F4, 0, 400); //E4
      tft.print("E ");
    }
    else if(InRange(freq, E4, F4U)){
      intonation = map(freq, E4, F4U, 0, 400); //F4
      tft.print("F ");
    }
    else if(InRange(freq, F4, G4)){
      intonation = map(freq, F4, G4, 0, 400); //F4U
      tft.print("F#");
    }
    else if(InRange(freq, F4U, G4U)){
      intonation = map(freq, B2, C3U, 0, 400); //G4
      tft.print("G ");
    }
    else if(InRange(freq, G4, A4)){
      intonation = map(freq, G4, A4, 0, 400); //G4U
      tft.print("G#");
    }
  }
  else{
